...
public:
	...
	void OnPayloadReceived(struct rte_mbuf **rsp, CustomAppHeader header, CTAConfig *conf, int ring_index);
	uint8_t TryGetData(rte_hash *logical_clocks, response r[MAX_PKT_BURST], uint8_t burst, LBInfo *conf);
