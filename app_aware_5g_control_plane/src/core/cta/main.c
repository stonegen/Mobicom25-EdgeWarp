...
const size_t GUTI_OFFSET = 
    sizeof(double) + sizeof(lgclock_t) + sizeof(size_t);
...
void* flush_tx(void *args) {
  ...
  while(!force_quit) {
    ...
    // thread.
    nb_rx =
          rte_ring_dequeue_burst(conf->tx_ring, (void **)pkt_burst, MAX_PKT_BURST, NULL);
...
void *processPkt(void *arg)
{
  ...
  int id = lb_args->id;
  struct rte_ring *rx_rings[lb_args->dequeue_ratios.size()];


  for (int i = 0; i < lb_args->dequeue_ratios.size(); i++){
    char ring_name[32];
    sprintf(ring_name, "Dequeue_%d_%d", lb_args->id, i); 
    rx_rings[i] = rte_ring_lookup(ring_name);
  ...
  while (!force_quit)
  {
    dequeue_counter = (dequeue_counter + 1) % 9;

    int sum = 0;
    for (int i = 0; i < lb_args->dequeue_ratios.size(); i++){
      if (dequeue_counter >= sum && 
          dequeue_counter <= sum + (lb_args->dequeue_ratios[i] - 1)){
        queue_id = i;
        break;
      }
      sum += lb_args->dequeue_ratios[i];
    }

    // Dequeueing messages coming from the 'recvPkts' thread.
    nb_rx =
        rte_ring_dequeue_burst(rx_rings[queue_id], (void **)pkt_burst, MAX_PKT_BURST, NULL);
    ...
    for (i = 0; i < nb_rx; i++)
    {
      ...
      nicInterface.OnPayloadReceived(rsp, header, lb_args->config, id);
    }
...
{
  struct LBInfo *info = (struct LBInfo*) arg;
  ...
  struct gutiExtractor ext;
  int queue_id = 0;
  const uint64_t hz = rte_get_timer_hz();
  lcore_id = rte_lcore_id();

  struct rte_ring **rx_rings = (struct rte_ring **) rte_malloc("Local_rings_recv", sizeof(struct rte_ring*), 0);

  for (int i = 0; i < RX_DEQUEUE_THREADS; i++){

    for (int j = 0; j < info->traffic_distribution.size(); j++){
      char ring_name[32];
      sprintf(ring_name, "Dequeue_%d_%d", i, j);
      (rx_rings + i)[j] = rte_ring_lookup(ring_name);
    } 
  ...
  while (!force_quit)
  {
    ...
    for (int pkt_id = 0; pkt_id < nb_rx; pkt_id++){
      struct rte_mbuf *m = pkt_burst[pkt_id];
      uint8_t *payload = rte_pktmbuf_mtod_offset(m, uint8_t *, PAYLOAD_OFFSET);

      for (int i = 0; i < sizeof(guti_t); i++){
        ext.buff[i] = *(payload + GUTI_OFFSET + i);
      } 

      int rem = ext.guti % 100; 
      int sum = 0;
      for (int i = 0; i < info->traffic_distribution.size(); i++){
        if (rem >= sum && rem <= sum + (info->traffic_distribution[i] - 1)){
          queue_id = i;
          break;
        }
        sum += info->traffic_distribution[i];
      }
      
      ret = rte_ring_enqueue((rx_rings + index)[queue_id], m);
...
int main(int argc, char **argv)
{
  ...
  Setting &traffic_dist = cfg.lookup("traffic_distribution");
  Setting &deq_ratios = cfg.lookup("dequeue_ratios");

  vector<int> traffic_distribution;
  for (int i = 0; i < traffic_dist.getLength(); i++){
    traffic_distribution.push_back(traffic_dist[i]);
  }

  vector<int> dequeue_ratios;
  for (int i = 0; i < deq_ratios.getLength(); i++){
    dequeue_ratios.push_back(deq_ratios[i]);
  }
  ...
  for (int i = 0; i < RX_DEQUEUE_THREADS; i++)
  {
    for (int j = 0; j < traffic_distribution.size(); j++){
      char ring_name[32];
      sprintf(ring_name, "Dequeue_%d_%d", i, j);
      struct rte_ring *r = rte_ring_create(ring_name, ring_size / 4, rte_socket_id(),
                                RING_F_SP_ENQ | RING_F_SC_DEQ);
    }


    struct LBInfo *info = (struct LBInfo *)rte_malloc(NULL, sizeof(LBInfo), 0);
    info->id = i;
    info->dequeue_ratios = dequeue_ratios;
    info->config = conf;
  ...
  struct LBInfo *recv_info = (struct LBInfo *) malloc(sizeof(LBInfo));
  recv_info->traffic_distribution = traffic_distribution;
  rte_eal_remote_launch((lcore_function_t *) recvPkts, recv_info, get_next_lcore(rte_socket_id()));
