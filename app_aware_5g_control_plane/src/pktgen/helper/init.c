...
static void pktgen_usage(const char *prgname) {
  printf(
      ...
      "  -t Traffic distribution: percentage traffic to allocate to each queue\n"
      "  -o Dequeue ratios: Dequeue slots assigned to each queue of a worker",
...
static int pktgen_parse_prioritization_arg(int *ary, char *c_arg) {
  char *token = strtok(c_arg, "_");
  int len = 0;
...
static const char short_options[] =
    ...
    "t:" /* percentage traffic to allocate to each queue */
    ;
  ...
  while ((opt = getopt_long(argc, argvopt, short_options, NULL,
    ...
    switch (opt) {
      ...
      case 's':
        ...
        break;
      case 't':
        conf->traffic_distribution = (int *)rte_malloc(NULL, sizeof(int), 0);
        conf->total_queues = pktgen_parse_prioritization_arg(conf->traffic_distribution, optarg);
        if (conf->total_queues == -1) {
          pktgen_usage(prgname);
          return -1;
        }
        break;
       case 'o':
        conf->dequeue_ratios = (int *)rte_malloc(NULL, sizeof(int), 0);
        conf->total_queues = pktgen_parse_prioritization_arg(conf->dequeue_ratios, optarg);
        if (conf->total_queues == -1) {
          pktgen_usage(prgname);
          return -1;
        }
