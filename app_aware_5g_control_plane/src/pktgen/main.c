...
static int run_worker(void *args) {
  ...
  while (!force_quit) {
    ...
    nb_rx = rte_ring_dequeue_burst(conf->rx_rings[0], (void **)pkts, MAX_PKT_BURST,
                                   NULL);
    if (nb_rx != 0) {
      stats->rx += nb_rx;
...
static int run_worker_bursty(void *args) {
  ...
  while (!force_quit) {
    ...
    int sum = 0;
    for (int i = 0; i < conf->total_queues; i++){
      if (dequeue_counter >= sum && 
          dequeue_counter <= sum + (conf->dequeue_ratios[i] - 1)){
        queue_id = i;
        break;
      }
      sum += conf->dequeue_ratios[i];
    }
    // Read packet burst from RX ring, generate responses and send them back
    nb_rx = rte_ring_dequeue_burst(conf->rx_rings[queue_id], (void **)pkts, MAX_PKT_BURST,
                                   NULL);
...
static int do_rx(void *args) {
  struct master_conf *conf = (struct master_conf *)args;
  uint64_t index;
  ...
  while (!force_quit) {
    ...
    for (int pkt_num = 0; pkt_num < nb_rx; pkt_num++){
      struct rte_mbuf *m = pkts_burst[pkt_num];
      ...
      for (int i = 0; i < conf->total_queues; i++){
        if (rem >= sum && rem <= sum + (conf->traffic_distribution[i] - 1)){
          queue_id = i;
          break;
        }
        sum += conf->traffic_distribution[i];
      }
      
    // Forwarding message burst to workers through their ring buffers.
      ret = rte_ring_enqueue(worker_conf[index].rx_rings[queue_id], (void *)pkts_burst);
      index = (index + 1) % conf->u_cores;
    }
...
static void config_worker(unsigned int id, int qid, guti_t guti_offset,
                          ...
                          serial_type serial_type, pcount_t proc_count,
                          int total_queues) {
  struct worker_conf *w = &worker_conf[id];

  w->id = id;

   w->rx_rings = (struct rte_ring **)rte_malloc(NULL, sizeof(struct rte_ring *), 0);

  for (int i = 0; i < total_queues; i++){
    char ring_name[64];
    sprintf(ring_name, "Worker_%d_rx_ring_%d", id, i);
    w->rx_rings[i] = rte_ring_create(ring_name, WORKER_RING_SIZE, rte_socket_id(),
                                RING_F_SC_DEQ);
...
int main(int argc, char **argv) {
  ...
  rte_eal_remote_launch(do_rx, (void *)conf, curr_core);

  // Check if uniform or burst and spawn the worker thread correspondingly
  printf("Proc type: %d", conf->proc_type);

  for (int i = 0; i < conf->u_cores; i++){
      worker_conf[i].dequeue_ratios = conf->dequeue_ratios;
      worker_conf[i].total_queues = conf->total_queues; 
  }
  ...
  if (conf->proc_count > 0) {
    ...
    for (i = 0; i < conf->u_cores; i++) {
      ...
      config_worker(i, i, nb_workers, conf->rate, conf->proc_type,
                    conf->serial_type, proc_share, conf->total_queues);
      get_next_lcore(&curr_core);
    ...
    for (i = 0; i < conf->u_cores; i++) {
      config_worker(i, i, nb_workers, conf->rate, conf->proc_type,
                    conf->serial_type, 0, conf->total_queues);
      get_next_lcore(&curr_core);
