...
pktgen_args = Template(
    "-d $duration -r $rate -u $workers -s $scheme -p $procedure -c $proc_count -t $traffic_distribution -o dequeue_ratios")
...
def gen_configs(config):
    ...
    pktgen_conf = {
        ...
        "traffic_distribution": "_".join(map(str, config["traffic_distribution"])),
        "dequeue_ratios": "_".join(map(str, config["dequeue_ratios"])),
