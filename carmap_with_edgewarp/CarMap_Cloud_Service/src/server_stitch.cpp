...
bool IsStateMigrationRequired;
bool IsDefaultMigrationModeSelected;
bool BlockingMigrationForDynamicStateOnly;
float MobilityHintTimeSeconds;
float MobilityHandoverTimeSeconds;
...
void ReadAndParseConfigFile(cv::FileStorage fSettings)
{
    ...
    IsStateMigrationRequired = ConvertStringToBool((string)fSettings["ServerStitch.IsStateMigrationRequired"]);
    IsDefaultMigrationModeSelected = ConvertStringToBool((string)fSettings["ServerStitch.IsDefaultMigrationModeSelected"]);
    BlockingMigrationForDynamicStateOnly = ConvertStringToBool((string)fSettings["ServerStitch.BlockingMigrationForDynamicStateOnly"]);
    MobilityHintTimeSeconds = std::stof(fSettings["ServerStitch.MobilityHintTimeSeconds"]);
    MobilityHandoverTimeSeconds = std::stof((string)fSettings["ServerStitch.MobilityHandoverTimeSeconds"]);
}
...
int main(int argc, char **argv)
{
    ...
    cout << "State migration enabled = " << IsStateMigrationRequired << ", Mode Default = " << IsDefaultMigrationModeSelected << endl;
    cout << "Mobility hint time (s) = " << MobilityHintTimeSeconds << " , Mobility handover time (s) = " << MobilityHandoverTimeSeconds << endl;
...
*/
    ...
    // ************************** EXTERNAN STORE RELATED UPDATES ************************//

    mtx = make_shared<mutex>();
    const std::string sourceIp = "127.0.0.1";
    const int sourcePort = 6379;

    const std::string destinationIp = "127.0.0.1";
    const int destinationPort = 6379;

    bool isRedisStore = true;
    bool isStateStoreRequired = true;

    Time::Init();
    shared_ptr<Store> stateStore;
    shared_ptr<StateTracker> stateTracker = make_shared<StateTracker>(mtx);
    shared_ptr<StateReplication> stateReplication = make_shared<StateReplication>(sourceIp, sourcePort, destinationIp, destinationPort, isRedisStore, stateTracker, mtx);

    if (isRedisStore)
    {
        cout << "Redis store selected." << endl;
        stateStore = make_shared<RedisStore>(sourceIp, sourcePort, stateTracker, true, mtx);
        stateStore->Connect();
    }
    else
    {
    ...
    vector<string> keys;
    keys.push_back("BaseMap");
    keys.push_back("StechedMap");

    stateTracker->AddKey(keys[0], 10);
    stateTracker->AddKey(keys[1], 10);

    bool isMobilityHintTriggered = false;
    bool isMobilityHandoverTriggered = false;

    // Variables used for default migration only.
    bool isDefaultMigrationTriggered = false;

    std::thread replicationThread;
    std::chrono::steady_clock::time_point blockingMigrationTimeStart;

    // Save base map to the external store
    boost::asio::streambuf baseMapBuffer;
    SerializeMap(baseMap->GetMap(), &baseMapBuffer);
    auto buf_size = baseMapBuffer.size();
    auto bufs = baseMapBuffer.data();
    std::string baseMapString(boost::asio::buffers_begin(bufs), boost::asio::buffers_begin(bufs) + buf_size);

    cout << "Basemap size = " << baseMapString.size() << endl;
    stateStore->Set(keys[0], baseMapString);

    auto simulationTimeStart = std::chrono::steady_clock::now();
    auto simulationTimeNow = std::chrono::steady_clock::now();
    auto simulationDurationSeconds = (std::chrono::duration_cast<std::chrono::microseconds>(simulationTimeNow - simulationTimeStart).count()) / 1000000.0f;
    ...
    while (true)
    {
        ...
        // cout << "***** The map size received is = " << recv_map_size_in_bytes << std::endl;
        ...
        cout << "Stiched map size  = " << write_buf.size() << endl;

        if (IsStateMigrationRequired)
        {
            simulationTimeNow = std::chrono::steady_clock::now();
            simulationDurationSeconds = (std::chrono::duration_cast<std::chrono::microseconds>(simulationTimeNow - simulationTimeStart).count()) / 1000000.0f;

            size_t buf_size = write_buf.size();
            boost::asio::streambuf::const_buffers_type bufs = write_buf.data();
            std::string stechedMapString(boost::asio::buffers_begin(bufs), boost::asio::buffers_begin(bufs) + buf_size);
            stateStore->Set(keys[1], stechedMapString);
            ...
            if (IsDefaultMigrationModeSelected == false)
            {
                ...
                {
                    cout << Time::GetCurrentTime() << ": Mobility hint event triggerred." << endl;
                    isMobilityHintTriggered = true;
                    replicationThread = std::thread(&StateReplication::SyncBackground, stateReplication, "Main");
                }

                // Mobility handover event
                if (simulationDurationSeconds > MobilityHandoverTimeSeconds && isMobilityHandoverTriggered == false)
                {
                    cout << Time::GetCurrentTime() << ": Mobility handover event triggerred." << endl;
                    isMobilityHandoverTriggered = true;
                    blockingMigrationTimeStart = std::chrono::steady_clock::now();
                    stateReplication->Notify();
                    replicationThread.join();
                    cout << Time::GetCurrentTime() << ": Mobility handover completed." << endl;
                }
            }
            else
            {
                if (simulationDurationSeconds > MobilityHandoverTimeSeconds && isDefaultMigrationTriggered == false)
                {
                    ...
                    // Now performm blocking migration with default scheme.

                    if (BlockingMigrationForDynamicStateOnly)
                    {
                        vector<string> dynamicKeysOnly;
                        dynamicKeysOnly.push_back(keys[1]);
                        stateReplication->Migrate(dynamicKeysOnly);
                    }
                    else
                    {
                        // Blocking migration of all the keys.
                        stateReplication->Migrate(keys);
                    }
                }
            }
        }
