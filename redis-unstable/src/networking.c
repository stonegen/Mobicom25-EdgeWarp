...
/* Adding Mutex here */
pthread_mutex_t redis_reply_mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t redis_reply_write_mutex = PTHREAD_MUTEX_INITIALIZER;
...
int writeToClient(client *c, int handler_installed) {
    ...
    pthread_mutex_lock(&redis_reply_write_mutex);
    ...
    if (nwritten == -1) {
        if (connGetState(c->conn) != CONN_STATE_CONNECTED) {
            ...
            freeClientAsync(c);
            pthread_mutex_unlock(&redis_reply_write_mutex);
            return C_ERR;
