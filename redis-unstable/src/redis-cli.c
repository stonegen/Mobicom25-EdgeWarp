...
{
    // printf("You got me bud here too ! 2 \n");
    char *command = argv[0];
    size_t *argvlen;
    int j, output_raw;
...
static int isSensitiveCommand(int argc, char **argv)
{
    if (!strcasecmp(argv[0], "auth"))
    {
        return 1;
             ...
             !strcasecmp(argv[0], "acl") && (!strcasecmp(argv[1], "deluser") || !strcasecmp(argv[1], "setuser") || !strcasecmp(argv[1], "getuser")))
    {
        return 1;
    }
    else if (argc > 2 &&
                               ...
                               dict *bigkeys_types_dict)
{
    ...
    while ((de = dictNext(di)))
    {
        ...
        if (memkey_type->count)
        {
            ...
            if (bk_de)
            { /* If we have it in memkeys it should be in bigkeys */
