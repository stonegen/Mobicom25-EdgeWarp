...
tpool_t *tm; // this will be the threadpool for the threads needed for migrating keys
tpool_t *tm2; // this will be the threadpool needs to serialise keys before the migration
dedicated_thread_t *dt;
...
int finishShutdown(void)
{
    ...
    if ((server.saveparamslen > 0 && !nosave) || save)
    {
        ...
        serverLog(LL_NOTICE, "Securely killing threadpool threads ...");
        
        tpool_wait(tm2);
        tpool_destroy(tm2);
        serverLog(LL_NOTICE, "Securely kiled tm2 threads ...");

        tpool_wait(tm);
        tpool_destroy(tm);
        serverLog(LL_NOTICE, "Securely kiled tm threads ...");

        stop_dedicated_thread(dt);
        serverLog(LL_NOTICE, "Securely kiled dt thread ...");
...
static void sigShutdownHandler(int sig)
{
    ...
    if (server.shutdown_asap && sig == SIGINT)
    {
        ...
        /*--> this is where we would like to cancel the all the threads*/
        serverLogRawFromHandler(LL_WARNING, "Waiting to kill threadpool threads....");
        // if(tm != NULL)
        // {
        //     tpool_wait(tm);
        //     tpool_destroy(tm);
        ...
        serverLogRawFromHandler(LL_WARNING, "threadpool threads killed successsfully....");
...
int main(int argc, char **argv)
{
    ...
    // int new_nice_value = -15; // Example value
    // set_process_priority(new_nice_value);


    pid_t pid = getpid(); // Get the current process ID
    struct sched_param param;

    // Set the priority to the maximum (99 for real-time scheduling)
    param.sched_priority = 99;

    // Set the real-time scheduling policy to SCHED_FIFO
    if (sched_setscheduler(pid, SCHED_FIFO, &param) == -1) {
        perror("sched_setscheduler failed");
        // exit(EXIT_FAILURE);
    }

    // Optionally, set the nice value (not generally needed for real-time scheduling)
    ...
    /* Here Shall we Create a thread pool for asynchronous migration :*/
    
    tm = tpool_create(1);
    tm2 = tpool_create(8);
    dt = create_dedicated_thread();
    serverLog(LL_NOTICE, "ThreadPool threads created successfully!");
